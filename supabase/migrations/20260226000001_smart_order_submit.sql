...
-- 1. Add status + submitted_at to smart_order_runs
ALTER TABLE public.smart_order_runs
  ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'draft',
  ADD COLUMN IF NOT EXISTS submitted_at TIMESTAMPTZ;

-- 2. Add source column to purchase_history
ALTER TABLE public.purchase_history
  ADD COLUMN IF NOT EXISTS source TEXT;

-- 3. Unique constraint: prevent duplicate PH rows per smart order run
ALTER TABLE public.purchase_history
  DROP CONSTRAINT IF EXISTS uq_purchase_history_smart_order_run;

ALTER TABLE public.purchase_history
  ADD CONSTRAINT uq_purchase_history_smart_order_run
  UNIQUE (restaurant_id, smart_order_run_id);

-- 4. RPC: submit_smart_order (idempotent upsert — safe to call multiple times)
